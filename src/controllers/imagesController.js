...
console.log("Usando el esquema de la base de datos:", connection.database); // para depuración

async function getDatabaseSize(databaseName) {
    return new Promise((resolve, reject) => {
      // Asegúrate de que databaseName es un string para evitar posibles ataques de inyección SQL
      ...
      connection.connect((err) => {
        ...
        // Usa el argumento databaseName directamente en la consulta
        const query = `
          SELECT SUM(data_length + index_length) AS 'size'
          FROM information_schema.TABLES
...
exports.getFolder = async (req, res) => {
  ...
  if (folderName === 'total') {
    ...
    let totalInfo = getTopLevelFolderInfo(imagesFolderPath); // Asumo que esta función ya está definida en alguna parte del código
  ...
  fs.access(folderPath, fs.constants.F_OK, (err) => {
    ...
    const folderContents = getFolderTotalSize(folderPath); // Asumo que esta función ya está definida en alguna parte del código
    res.json({
      folderName,
